...
/**
 * A faint field of flowing contour lines — the wood-grain / topographic reading
 * of the Tenure rings. Deterministic (seedable), inherits color via currentColor,
 * meant to sit very faintly behind a section. Decorative only.
 */
export function ContourField({
  ...
  const paths = Array.from({ length: lines }, (_, i) => {
    ...
    for (let s = 1; s <= steps; s++) {
      const t = s / steps;
      const x = W * t;
      const taper = Math.sin(t * Math.PI); // settle toward the edges
      const y =
        baseY +
        slope * x +
